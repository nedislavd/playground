...
const calcHeightOfChar = (char: string): number => char.charCodeAt(0) - 97;
...
const part1 = (rawInput: string) => {
  ...
  const map = input.map((row) =>
    row.map((point) => {
      if (point === "S")
        return {
          height: calcHeightOfChar("a"),
          isEnd: false,
        } as Point;
      else if (point === "E")
        return {
          height: calcHeightOfChar("z"),
          isEnd: true,
        } as Point;
      return {
        height: calcHeightOfChar(point),
        isEnd: false,
      } as Point;
    }),
  );

  return dijkstraNoCostMap(start, end, map);
};

const dijkstraWithCostMap = (
  start: Position,
...
const part2 = (rawInput: string) => {
  ...
  return dijkstraWithCostMap(end, starting, map);
};
